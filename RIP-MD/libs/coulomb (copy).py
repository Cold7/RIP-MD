...
def potentialByFrame(parameters):
	...
	selection = None
	try:
		selection=u.select_atoms("protein").residues
	except:
		selection=u.selectAtoms("protein").residues
...
def potential(i):
	...
	nodeList=[]
	groupList1=[]
	resNode1=sel[i].resname+":"+sel[i].segment.segid+":"+str(sel[i].resid)
	...
	j=i+1
	while j<len(sel):
		resNode2=sel[j].resname+":"+sel[j].segment.segid+":"+str(sel[j].resid)		
		nodeList2=[]
		groupList2=[]		
		#geting node name (that is equal to atomindex +1 cause it begin from 0)
		for atom in sel[j]:
...
def parse(pdbNames, output, nproc, Nodes, Topology, Ecs, Erf, T, Krf, Rrf_threshold, coulomb_excluded, reactionField):
	global coulombCte, sel, resNumbers, topology, excluded, positionDict, Rrf,Crf,KbT, nodes, outputFolder, RF
	RF=reactionField
	outputFolder=output
	nodes=Nodes
...
#		sel=u.select_atoms("protein").residues
