...
def parse(pdbNames, output, nproc, Nodes, Topology, Ecs, Erf, T, Krf, Rrf_threshold, coulomb_excluded, reactionField, selection):
	global coulombCte, sel, resNumbers, topology, excluded, positionDict, Rrf,Crf,KbT, nodes, outputFolder, RF, sel
	sel = selection
	RF=reactionField
	outputFolder=output
	nodes=Nodes
...
####
		#try:
		#	sele=u.select_atoms(selection).residues
		#except:
		#	sele=u.selectAtoms(selection).residues
